...
\begin{table}[htbp]
\centering
\caption{Ablation results (mean $\pm$ std over 5 seeds) under simulated LiveMCPBench conditions.\protect\footnotemark}
...
\hline
\end{tabular}
\footnotetext{Tasks are generated by sampling real tools from the MCP manifest and paraphrasing their descriptions. Results are mean $\pm$ std over 5 random task seeds. These reflect \emph{simulated benchmark conditions}; they are not obtained from the official Docker-based evaluation harnesses.}
\end{table}


\begin{table}[htbp]
\centering
\caption{Ablation results (mean $\pm$ std over 5 seeds) under simulated MCP-Bench conditions.\protect\footnotemark}
...
\hline
\end{tabular}
\footnotetext{Results are mean $\pm$ std over 5 random task seeds under \emph{simulated benchmark conditions}.}
\end{table}

\subsection{Analysis}

\paragraph{Saturated Retrieval Quality on Simulated Queries.}
As shown in Tables~\ref{tab:ablation_livemcpbench} and~\ref{tab:ablation_mcp_bench}, under simulated benchmark conditions where task queries are natural-language paraphrases of tool descriptions, dense semantic search is already near-perfect, achieving a mean Recall@5 of $0.996 \pm 0.005$ and $1.000 \pm 0.000$ on LiveMCPBench and MCP-Bench respectively. Under these circumstances, there is little margin to show retrieval quality gains over the baseline. However, the true utility of FED-GRAPH-MCP is not merely raw retrieval recall under idealized queries, but rather its \textbf{systems efficiency}, \textbf{online synchronization}, and \textbf{structural graph maintenance} capabilities.

\paragraph{RQ1: Systems Efficiency via Token Savings.}
In large-scale agent systems, injecting full details of all retrieved candidate tools into the LLM context is highly expensive and exceeds token budgets. By combining hybrid retrieval with graph expansion (\texttt{rrf\_expand} and the full pipeline), FED-GRAPH-MCP enables the agent to start with a much smaller candidate set and dynamically pull relevant functional dependencies. As shown in the ``Tokens/Task'' column of Tables~\ref{tab:ablation_livemcpbench} and~\ref{tab:ablation_mcp_bench}, our graph expansion variants reduce the average token footprint per task from 1,250 tokens (for top-5 candidates) to 1,168--1,190 tokens (a 4.8--6.6\% reduction) while preserving $\sim$98--100\% Recall@5 and $\sim$89--94\% success rate. This results in significant cost savings at scale.

\paragraph{RQ2: Fusion and Expansion Dynamics.}
GNN-Only retrieval achieves a mean R@1 of $0.261 \pm 0.036$ / $0.290 \pm 0.044$ (LiveMCPBench/MCP-Bench), confirming that the structural embeddings capture complementary signal but are biased toward topological proximity rather than exact semantic matching. When fused via RRF, Recall@1 rises to $0.914 \pm 0.026$ / $0.923 \pm 0.014$. Adding graph expansion over the top seeds further improves Recall@1 to $0.945 \pm 0.028$ / $0.939 \pm 0.013$ while simultaneously reducing token counts.
...
\paragraph{RQ5: Graph density vs.\ systems cost trade-off.}
Fig.~\ref{fig:density_sweep} visualizes the relationship between
graph edge density (controlled by edge mining thresholds), estimated
graph memory footprint, and GNN-Only Recall@1 (mean over 3 seeds).
As the edge threshold is relaxed from 0.9 to 0.1, edge count grows
from 11K to 99K and memory from 5.2\,MB to 13.7\,MB, while GNN-Only
R@1 initially rises (peaking near 28\% at $\sim$55K edges) before
declining due to over-smoothing. This confirms an optimal
sparsification ``sweet spot'' that maximizes structural signal while
minimizing systems overhead.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{density_vs_memory_recall.png}
\caption{Graph density sweep: GNN-Only Recall@1 (\%) and estimated
graph memory (MB) as a function of edge count. The optimal operating
point balances structural signal against memory overhead and
over-smoothing.}
...
benchmark distributions: FED-GRAPH-MCP achieves mean Recall@1 of
$0.724 \pm 0.053$ vs.\ $0.764 \pm 0.031$ and nDCG@5 of
$0.878 \pm 0.026$ vs.\ $0.893 \pm 0.010$ on LiveMCPBench and MCP-Bench
respectively. This suggests that the functional graph and fusion
mechanism generalize across different task distributions rather than
overfitting to one benchmark's characteristic query style, even when
evaluated under differing paraphrase templates, tool pools, and random
seeds.
