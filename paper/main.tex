...
\begin{enumerate}
    ...
    \textsc{param\_type\_overlap}, \textsc{compose\_dep})
    ...
    tools, achieving 83.9--99.9\% latency reduction over full
    rebuilds.
...
\begin{itemize}
    ...
    \item \textbf{\textsc{param\_type\_overlap}}: Tools share
    $\geq 2$ parameter types (e.g., both accept
    \texttt{datetime}, \texttt{location}). Jaccard similarity over
    parameter type sets $> 0.4$.
...
\midrule
\textsc{co\_invoke}          & 1,842 & 0.61 \\
\textsc{schema\_compat}      & 3,104 & 0.74 \\
\textsc{param\_type\_overlap}& 5,217 & 0.52 \\
\textsc{compose\_dep}        & 987   & 0.83 \\
\midrule
Total                         & 11,150 & --- \\
\bottomrule
\end{tabular}
...
descriptions. We use a 2-layer R-GCN with hidden dimension 256 and
...
\begin{enumerate}
    ...
    $f\!: \mathbb{R}^{769} \to \mathbb{R}$ taking
    $[e_q \| e_t \| s_{\text{RRF}}]$ as input, trained on 493
    query-tool positive pairs from APIBank with pairwise margin loss
    (final loss: 0.0567).
\end{enumerate}
...
\midrule
\texttt{server\_add}    & 1,173 & 7,908 & 85.2\% \\
\texttt{tool\_update}   & 1,270 & 7,908 & 83.9\% \\
\texttt{server\_remove} & 4     & 7,908 & 99.9\% \\
\bottomrule
\end{tabular}
\end{table}
...
graph construction (4,739\,ms) + embedding generation (1,998\,ms) +
GNN forward pass (1,002\,ms) + FAISS build (170\,ms) =
\textbf{7,908\,ms} total.
...
Table~\ref{tab:sync_latency} demonstrates 83.9--99.9\% latency
reduction across all event types. The \texttt{server\_remove} event
is fastest (4\,ms) because no embedding recomputation is
required---only node/edge deletion from the graph and FAISS
\texttt{remove\_ids()}. The \texttt{tool\_update} event (1,270\,ms)
is the most expensive incremental operation due to edge recomputation
and 2-hop GNN refresh, but still $6.2\times$ faster than a full
...
reduces re-indexing latency by 83.9--99.9\% versus full rebuilds.
